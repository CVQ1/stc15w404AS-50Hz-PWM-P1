...
void timer0_int (void) interrupt TIMER0_VECTOR
{
   //P25 = ~P25;
   	times++;
    if(times==2000) times=0;	 //
	if(times==50) P1=0xff;
	if(times>=50&&times<=250)
	{
	if(times==pwm[0]) P10=~P10;
	if(times==pwm[1]) P11=~P11;
	if(times==pwm[2]) P12=~P12;
	if(times==pwm[3]) P13=~P13;
	if(times==pwm[4]) P14=~P14;
	if(times==pwm[5]) P15=~P15;
	if(times==pwm[6]) P16=~P16;
	if(times==pwm[7]) P17=~P17;
	}
